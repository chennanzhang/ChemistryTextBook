...
其次，我们用摩尔来衡量双原子分子或多原子分子构成的各种物质的时候，那么同样地可以推知，\qty{1}{mol} 任何分子的质量，就是以克为单位，数值上等于该种分子的式量。
\begin{itemize}[label={},labelsep=0pt]
  \item 氢气的式量是 2，\qty{1}{mol} 氢气的质量是 \qty{2}{g}，
  \item 氧气的式量是 32，\qty{1}{mol} 氧气的质量是 \qty{32}{g}，
  \item 二氧化碳的式量是 44，\qty{1}{mol} 二氧化碳的质量是 \qty{44}{g}，
  \item 水的式量是 18，\qty{1}{mol} 水的质量是 \qty{18}{g}。
\end{itemize}
...
\end{example}
\begin{solution}
  硫酸的式量是 98，硫酸的摩尔质量是 \qty{98}{g/mol}。
  \[\begin{split} 
...
例如，\qty{20}{\celsius} 时，\qty{1}{mol} 铁的体积是 \qty{7.1}{cm^3}，\qty{1}{mol} 铝的体积是 \qty{10}{cm^3}，\qty{1}{mol} 铅的体积是 \qty{18.3}{cm^3}（\cref{fig:1-1}）；\qty{1}{mol} 水的体积是 \qty{18.0}{cm^3}， \qty{1}{mol} 纯硫酸的体积是 \qty{54.1}{cm^3}，\qty{1}{mol} 蔗糖的体积是 \qty{215.5}{cm^3}（\cref{fig:1-2}）。
\begin{figure}
...
\begin{Theorem}{讨论}
如果已经知道水的化学式是 \ce{H2O}，我们能够根据氢气跟氧气化合成水蒸气的体积比是 $2:1:2$，应用阿伏伽德罗定律来证明 1 个氧分子里含有 2 个氧原子吗？
\end{Theorem}
...
\begin{example}
  在标准状况时，\qty{0.20}{L} 的容器里所含一氧化碳的质量为 \qty{0.25}{g}，计算一氧化碳的式量。
\end{example}

根据摩尔体积可以计算出一氧化碳的摩尔质量，而摩尔质量的数值就等于它的式量。
...
\begin{Practice}[习题]
  \begin{question}
    ...
    \item 在标准状况时，\qty{235}{ml} 某种气体的质量是 \qty{0.406}{g}，计算这种气体的式量。
  \end{question}
\end{Practice}
...
\begin{solution}
\ce{HCl} 的式量是 36.5，它的摩尔质量是 \qty{36.5}{g/mol}。\qty{0.73}{g} \ce{HCl} 相当于：
...
\end{example}
\begin{solution}
  \ce{NaOH} 的式量是 40，它的摩尔质量是 \qty{40}{g/mol}。
...
\end{example}
\begin{solution}
  蔗糖 \ce{C12H22O11} 的式量是 342，蔗糖的摩尔质量是 \qty{342}{g/mol}。
...
在计算燃烧热时，可燃物质是以 \qty{1}{mol} 作为标准来计算的，所以热化学方程式里的元素符号或化学式前面的系数可以用分数表示。
...
\begin{align*}
  \text{物质的量}(\unit{mol})&=\frac{\text{物质的质量}(\unit{g})}{\text{物质的摩尔质量}(\unit{g/mol})}\\
  \text{气体物质的量}(\unit{mol})&=\frac{\text{气体的体积}(\unit{L})}{\text{气体的摩尔体积}(\qty{22.4}{L/mol})}\\
  \text{摩尔浓度}(\unit{M})&=\frac{\text{溶质的量}\unit{mol}}{\text{溶液的体积}(\unit{L})}
\end{align*}
...
\begin{Review}
  \begin{question}
    ...
    \item 称取 \qty{1.721}{g} 某种硫酸钙的结晶水合物 $\ce{CaSO4 .x H2O}$， 加热，使它失去全部结晶水。这时候硫酸钙的质量是 \qty{1.361}{g}，计算所含结晶水数（$x$）。
