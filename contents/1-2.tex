...
在 107 种元素\footnote{本书初版时，被发现的元素仅有 107 种，而到 2016 年，被确定存在的元素已经有 118 种。}的原子里，还有溴、碘、砹的原子结构跟氟和氯相似，在最外层都有 7 个电子。
氟、氯、溴、碘、砹\footnote{鿫是2015年被发现，2016年被命名，这里尊重原文未将其放在其中。}具有相似的化学性质，成为一族，称为卤族元素，简称卤素。
砹在自然界里含量很少。
...
氯气（\ce{Cl2}）的分子是由两个氯原子\footnotemark[3]组成的双原子分子（\cref{fig:2-1}）。 
...
\end{figurehere}
\end{minipage}
\footnotetext[3]{氯原子很小，它的原子半径，即氯分子中两个原子核间距离的一半，是 \qty{0.99e-10}{m}。}
...
\begin{Experiment}*
  把一束细铜丝灼热后，立刻放进盛有氯气的集气瓶里（\cref{fig:2-3}），观察发生的现象。把少量的水注入集气瓶里，用毛玻璃片把瓶口盖住，振荡。观察溶液的颜色。
  \tcblower
  \begin{figurehere}
...
这个反应可以用化学方程式表示如下：
\[\schemestart 
\ce{ Cu} \+ \ce{Cl2} $\xlongequal{\triangle}$ \chemname{\ce{CuCl2}}{\footnotesize 氯化铜}
...
把新收集的一瓶氯气和一瓶氢气（氢气和氯气可以分别收集在透明或半透明的塑料制的集气瓶里），口对口地对着，抽去瓶口间的玻璃片，上下颠倒几次，使氯气和氢气充分混和。
...
\begin{gather*}
  \schemestart
  \ce{2P} \+ \ce{3Cl2} $\xlongequal{\text{点燃}}$
  2\chemname{\ce{PCl3}}{\footnotesize 三氯化磷}
  \schemestop\chemnameinit{}\\
  \schemestart
  \ce{PCl3} \+ \ce{Cl2} $\xlongequal{\quad}$
  \chemname{\ce{PCl5}}{\footnotesize 五氯化磷}
  \schemestop\chemnameinit{}
  % \ce{ 2P +3Cl2  2PCl3 }\\
...
\schemestart
  \ce{Cl2} \+ \ce{H2O} $\xlongequal{\quad}$
  \chemname{\ce{HCl}}{\footnotesize 盐酸} \+
  \chemname{\ce{HClO}}{\footnotesize 次氯酸} 
\schemestop\chemnameinit{}
\]
...
\begin{Experiment}*[righthand ratio=0.6]
  ...
  \end{figurehere}
\end{Experiment}
\[ \ce{ 2HClO \xlongequal{\text{光照}} 2HCl + O2 ^ }\]
...
\begin{Experiment}
  ...
  \tcblower
  \begin{figurehere}
    \includegraphics{2-7.pdf}\par
    {\footnotesize\begin{enumerate*}[a:]\item 湿润的有色布条\hspace*{2em}\item 干燥的有色布条\end{enumerate*}}
    \caption{次氯酸使色布褪色}\label{fig:2-7}
  \end{figurehere}
\end{Experiment}
可以看到湿润的有色布条褪了色，干燥的却没有（\cref{fig:2-7}）。可见起漂白作用的是次氯酸。
...
\ce{ 2NaOH } \+  \ce{Cl2 }
$\xlongequal{\quad}$
\ce{ NaCl} \+ \chemname{\ce{NaClO}}{\footnotesize 次氯酸钠} \+ \ce{H2O } 
\schemestop\chemnameinit{}\]
...
\ce{ 2Ca(OH)2} \+ \ce{2Cl2} 
$\xlongequal{\quad}$ 
\chemname{\ce{Ca(ClO)2}}{\footnotesize 次氯酸钙} \+ \chemname{\ce{CaCl2}}{\footnotesize 氯化钙} \+ \ce{2H2O} 
\schemestop\chemnameinit{}\]

漂白粉应用于漂白的时候，使次氯酸钙跟稀酸或空气里的二氧化碳和水蒸气起反应，就生成次氯酸。
\begin{gather*}
  \schemestart
  \ce{ Ca(ClO)2} \+ \ce{2HCl} $\xlongequal{\quad}$ \chemname{\ce{CaCl2}}{\footnotesize 氯化钙} \+ \ce{2HClO } \schemestop\chemnameinit{} \\ \schemestart
  \ce{ Ca(ClO)2} \+ \ce{CO2} \+ \ce{H2O} $\xlongequal{\quad}$ \chemname{\ce{CaCO3}}{\footnotesize 碳酸钙} \ce{v} \+ \ce{2 HClO }
  \schemestop\chemnameinit{}
\end{gather*}
\subsection{氯气的用途}
...
\begin{Experiment}
  象\cref{fig:2-8} 所示那样把装置连接好，检查气密性。在烧瓶里加入少量二氧化锰粉末，从分液漏斗慢慢地注入密度为 \qty{1.19}{g/cm^3} 的浓盐酸。缓缓加热，使反应加速，氯气就均匀地放出。用向上排空气法收集，多余的氯气用氢氧化钠吸收。
  \tcblower
  \begin{figurehere}
...
这个反应可以用化学方程式表示如下：
\[ \schemestart
\ce{ 4HCl} \+ \chemname{\ce{MnO2}}{\footnotesize 二氧化锰} $\xlongequal{\triangle}$ \chemname{\ce{MnCl2}}{\footnotesize 二氯化锰} \+ \ce{2H2O} \+ \ce{Cl2 ^}
\schemestop\chemnameinit{}\]
...
\[\schemestart 
\ce{NaCl} \+ \ce{H2SO4} \text{（浓）} $\xlongequal{\quad}$ \chemname{\ce{NaHSO4}}{\footnotesize 硫酸氢钠} \+ \chemname{\ce{HCl}}{\footnotesize 氯化氢} \ce{ ^ } \schemestop\chemnameinit{}\]
在 \qtyrange{500}{600}{\celsius} 的条件下，继续起反应而生成硫酸钠和氯化氢。
\[\schemestart \ce{NaHSO4} \+ \ce{NaCl} $\xlongequal{\triangle}$ \chemname{\ce{Na2SO4}}{\footnotesize 硫酸钠} \+ \ce{HCl ^} 
\schemestop\chemnameinit{}\]
...
\begin{Practice}[习题]
  \begin{question}
    \item \qty{11.2}{L} 氧气和 \qty{11.2}{L} 氢气起反应，生成多少升的氯化氢气体（气体体积都按标准状况计），把生成的氯化氢都溶解在 \qty{328.5}{g} 水里形成密度为 \qty{1.047}{g/cm^3} 的盐酸，计算这种盐酸的摩尔浓度。
    ...
    \begin{tasks}(3)
      ...
      \task \ce{MgCO3}
    \end{tasks} 
    \item \qty{11.7}{g} 氯化钠跟 \qty{10}{g} 浓度为 98\% 的硫酸反应，微热时生成多少克氯化氢？继续加热到 \qty{600}{\celsius} 时，又生成多少克的氯化氢？
...
\[
  2\OX{a,Na} + \OX{b,Cl}_2 \xlongequal{\quad} 2\OX{c,Na}\OX{d,Cl}
  \redox(a,c)[->]{\scriptsize 化合价升高，被氧化}
  \redox(b,d)[->][-1]{\scriptsize 化合价降低，被还原}
  \rule[-22pt]{0pt}{52pt}
\]
\[
  \OX{a,H}_2 + \OX{b,Cl}_2 \xlongequal{\quad} 2\OX{c,H}\OX{d,Cl}
  \redox(a,c)[->]{\scriptsize 化合价升高，被氧化}
  \redox(b,d)[->][-1]{\scriptsize 化合价降低，被还原}
  \rule[-22pt]{0pt}{52pt}
\]
...
在下面的化学方程式里，“\ce{e}” 表示电子，并用箭头表明同一元素的原子得到或失去电子的情况。
\[
  2\OX{a,Na} + \OX{b,Cl}_2 \xlongequal{\quad} 2\OX{c,Na}\OX{d,Cl}
  \redox(a,c)[->]{\scriptsize 失去 \ce{2e}}
  \redox(b,d)[->][-1]{\scriptsize 得到 \ce{2e}}
  \rule[-20pt]{0pt}{45pt}
\]
\[
  \OX{a,Cu} + \OX{b,Cl}_2 \xlongequal{\quad} 2\OX{c,Na}\OX{d,Cl}
  \redox(a,c)[->]{\scriptsize 失去 \ce{2e}}
  \redox(b,d)[->][-1]{\scriptsize 得到 \ce{2e}}
  \rule[-20pt]{0pt}{45pt}
\]
...
在反应过程里，钠原子失去 1 个电子，钠从 0 价变到 $+1$ 价，化合价升高；氯原子得到 1 个电子，氯从 0 价变到 $-1$ 价，化合价降低。
...
\[
  \vspace{7mm}
  \schemestart
  2\chemname{\OX{a,Na}}{\footnotesize 还原剂} \+ 
  \chemname{\OX{b,Cl}}{\footnotesize 氧化剂}$_2$ 
  $\xlongequal{\quad}$
  2\ce{NaCl}
  \redox(a,b)[->]{\scriptsize \ce{2e}}
  \schemestop\chemnameinit{}
\]
\[
  \vspace{4mm}
  \schemestart
  \chemname{\OX{a,Cu}}{\footnotesize 还原剂} \+ 
  \chemname{\OX{b,Cl}}{\footnotesize 氧化剂}$_2$ 
  $\xlongequal{\quad}$
  \ce{CuCl2}
  \redox(a,b)[->]{\scriptsize \ce{2e}}
...
\[ 
\ce{H}\:\electformular{Cl}{d}{x}{d}{d}{d}{d}{d}{d}
\]
...
\begin{solution}
  ...
  \[
  \vspace{7mm}
  \OX{a,Mg} + 2\OX{b,H}\ce{Cl} \xlongequal{\quad} 2\OX{c,Mg}\ce{Cl2} + \OX{d,H}_2
  \redox(a,c)[->]{\scriptsize 失去 \ce{2e}，化合价升高，被氧化}
  \redox(b,d)[->][-1]{\scriptsize 得到 \ce{2e}，化合价降低，被还原}
  \]
...
\begin{sidewaystable}
  ...
  \begin{tblr}{colspec={cccccX[l]cS[table-format=4.2,table-number-alignment=right]S[table-format=4.2,table-number-alignment=right]c},hline{2}=0.8pt,row{1}={m,c,guard},stretch=3.5}
    {元素\\名称} & {元素\\符号} & {核电\\荷数} & 电子层结构 & 单质 & 颜色和状态 & {密度} & {熔点\\（\unit{\celsius}）}& {沸点\\（\unit{\celsius}）} & {溶解度\\（\qty{100}{g} 水）}\\
    氟 & \ce{F}  & 9  & \Dzc{2,7} & \ce{F2} & 淡黄绿色气体 & \qty{1.69}{g/L} & -188.1 & -219.6 & 反应  \\
    氯 & \ce{Na} & 17 & \Dzc{2,8,7} & \ce{Cl2}  & 黄绿色气体 & \qty{3.214}{g/L}  & -34.6 & -101 &  \qty{226}{cm^3} \\
    溴 & \ce{K}  & 35 & \Dzc{2,8,18,7} & \ce{Br2}  & 深棕红色液体 & \qty{3.119}{g/cm^3}  & 58.78 & -7.2 &  \qty{4.17}{g} \\
    碘 & \ce{Rb} & 53 & \Dzc{2,8,18,18,7} & \ce{I2}  & 紫黑色固体 & \qty{4.93}{g/cm^3} & 184.4 & 113.5 & \qty{0.029}{g}  \\
  \end{tblr}\par\smallskip
  \begin{minipage}{\linewidth}
    \footnotesize 注：密度或溶解度是指常温时的数据。
  \end{minipage}
\end{sidewaystable}
...
\begin{Experiment}
  把水注入盛着少量溴的试管，振荡，水溶液的颜色显橙色（\cref{fig:2-14},\MyRoman{1}）。把上部橙色溶液倒在另一个试管里，再注入少量无色的汽油（或苯或四氯化碳）（\cref{fig:2-14},\MyRoman{2}），用力振荡，静置一会儿。观察油层和水溶液的颜色。
  \tcblower
  \begin{figurehere}
...
\begin{Experiment}
  在 1 个铁坩埚里放锌粉 \qty{0.5}{g}，加入碘粉 \qty{0.5}{g}，混合后，加水 1～2 滴作为催化剂，观察发生的现象。
\end{Experiment}
...
\[\schemestart
\ce{H2}\,\text{（气）} \+ \ce{F2}\,\text{（气）} $\xlongequal{\quad}$ \chemname{\ce{2HF}}{\footnotesize 氟化氢}\,\text{（气）} \+ \qty{128.4}{kCal}
\schemestop\chemnameinit{}\]

溴的性质不如氯活泼，溴跟氢气的反应在达到 \qty{500}{\celsius} 时即较慢地进行。
\[\schemestart
\ce{H2}\,\text{（气）} \+ \ce{Br2}\,\text{（气）} 
$\xlongequal{\quad}$ 
\chemname{\ce{2HBr}}{\footnotesize 溴化氢}\,\text{（气）} \+ \qty{17.3}{kCal}
\schemestop\chemnameinit{}\]

碘的性质比溴更不活泼，碘跟氢气的反应在不断加热条件下缓慢地进行，生成的碘化氢很不稳定，同时发生分解。

\[\schemestart
\ce{H2}\,\text{（气）} \+ \ce{I2}\,\text{（固）} $\xlongequal{\triangle}$ 
\chemname{\ce{2HI}}{\footnotesize 碘化氢}\,\text{（气）} $-$ \qty{12.4}{kCal}
\schemestop\chemnameinit{}\]
...
\begin{table}
  ...
  \begin{tblr}{colspec={cX[3,l]X[2,l]X[3,l]},hline{2}=0.8pt,row{1}={m,c}}
    化学式 & 跟氢气的反应和氢化物的稳定性 & 跟水的反应 & 卤素的活动性比较 \\
    \ce{F2}  & 在冷暗处就能剧烈化合而爆炸，\ce{HF} 很稳定。 & 使水迅速分解，放出氧气。 & 氟最活泼，能把氯、溴、碘从它们的化合物中置换出来。\\
    \ce{Cl2} & 在强光照射下，剧烈化合而爆炸，\ce{HCl} 较稳定。 & 在日光照射下，缓慢放出氧气。& 氯次之，能把溴、碘从它们的化合物中置换出来。\\
    \ce{Br2} & 在高温条件下，较慢地化合，\ce{HBr} 较不稳定。 & 反应较氯较为弱。   & 溴又次之，能把碘从它的化合物中置换出来。\\
    \ce{I2}  & 持续加热，慢慢地化合，\ce{HI} 很不稳定，同时发生分解。 & 只起很微弱的反应。 & 碘较不活泼。\\
  \end{tblr}
\end{table}
...
\begin{gather*}
  \schemestart
  \ce{ NaBr} \+ \ce{AgNO3} $\xlongequal{\quad}$ \chemname{\ce{AgBr}}{\footnotesize 溴化银} \ce{ v } \+ \ce{NaNO3}
  \schemestop\chemnameinit{}\\
  \schemestart
  \ce{ KI} \+ \ce{AgNO3} $\xlongequal{\quad}$ \chemname{\ce{AgI}}{\footnotesize 碘化银} \ce{ v } + \ce{KNO3 }
  \schemestop\chemnameinit{}
\end{gather*}
